...
Page({
  data: {
    ...
    payTypeList: '',
    motionList: [ //眼球运动
      { num: '1', value: '正常', checked: false },
      { num: '2',value: '卡顿', checked: false }],
    coverList: [  //遮盖眼位
      { num:'1', value: '正常', checked: false },
      { num: '2',value: '由外向内', checked: false },
      { num: '3',value: '由内向外', checked: false },
    ],
    colourVisionList: [ //色觉检查
      { num: '1',value: '正常', checked: false },
      { num: '2',value: '红色盲', checked: false },
      { num: '3',value: '绿色盲', checked: false },
      { num: '4', value: '红色弱', checked: false },
      { num: '5', value: '绿色弱', checked: false },
    ]
  ...
  motionInput(e) {
    ...
    const motionList = this.data.motionList;
    for (let i = 0, len = motionList.length; i < len; ++i) {
      motionList[i].checked = motionList[i].value === e.detail.value
    }
    this.setData({
      motionList
    })
  ...
  coverInput(e) {
    ...
    const coverList = this.data.coverList;
    for (let i = 0, len = coverList.length; i < len; ++i) {
      coverList[i].checked = coverList[i].value === e.detail.value
    }
    this.setData({
      coverList
    })
  ...
  colourVisionInput(e) {
    ...
    const colourVisionList = this.data.colourVisionList;
    for (let i = 0, len = colourVisionList.length; i < len; ++i) {
      colourVisionList[i].checked = colourVisionList[i].value === e.detail.value
    }
    this.setData({
      colourVisionList
    })
